...
```bash
docker save mf-render:0.2.3 | gzip -1 | ssh <算力机> 'gunzip | sudo docker load'
```
...
```bash
ssh <算力机> 'sudo docker rm -f mf-render && sudo docker rmi mf-render:0.2.3'
```
