...
docker build --platform linux/amd64 -t mf-render:0.3.0 .
```
...
```bash
docker save mf-render:0.3.0 | gzip -1 | ssh <算力机> 'gunzip | sudo docker load'
```
...
```bash
ssh <算力机> 'sudo docker rm -f mf-render && sudo docker rmi mf-render:0.3.0'
```
