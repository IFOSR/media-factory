...
| # | 决策 | 结论 |
|---|---|---|
| 1 | 分镜是否独立成第 5 步 | **是**（`cover` 模式自动跳过该步） |
| 2 | 算力机不可用时的默认行为 | **`cover`（先保证交付）**：立即出静态封面版；算力机恢复后可重跑 video 升级为动态版 |
| 3 | 成品规格 | **1080p / 24fps / CRF16**（4.4 分钟约 15~30MB） |
| 4 | spike 样片视觉 | **通过**，模板库按此视觉定稿 |
| 5 | 安装分发 | `install.sh` 保持轻量；Release 维持四平台单文件；渲染栈只装算力机 |
| 6 | 算力机环境隔离 | **用 Docker 容器**，不改动宿主环境（宿主已有 vLLM 服务在跑） |
| 7 | 默认画面模式 | `dynamic`（动态解释画面）；算力机不可用时降级 `cover` |

### 关键指标（实测基准）

| 指标 | 实测值 |
|---|---|
| 渲染 20s 1080p（容器内，24fps/16w） | 7.3 秒 |
| 外推 4.4 分钟音频 | 约 1.5~2 分钟 |
...
**现象**：8.6 分钟播客（202 段字幕）执行分镜时报「模型输出中未找到 JSON 分镜」→ 自动降级封面模式。

**根因**：一次性把整篇字幕交给 LLM 并要求输出覆盖全片的分镜 JSON，**输出规模过大被截断**（或模型改用散文回答），拿不到完整 JSON。

**修复**（`src/cmd/scenes.rs`）：
- **分块规划**：字幕按 50 段一批（`CHUNK_ENTRIES`），每批只要求覆盖该索引区间（提示词新增 `{{RANGE}}`），最后合并 + 统一校验；末批过小则并入前一批
- **失败重试**：解析失败时带「只输出完整合法 JSON」的修复提示重试一次
- **可诊断**：解析失败时把模型输出片段写入思考链路日志（此前完全看不到原因）
- **提示词收紧**：单批输出不超过 12 个场景，避免再次超长

**验证**（对同一条失败任务重跑）：
| | 修复前 | 修复后 |
|---|---|---|
| 分镜 | failed | **done：24 场景 / 6 种类型（含 bar_chart）**，覆盖 0→201 全段 |
| 视频 | 降级封面 | **动态画面 1080×1920（跟随 9:16 配图）**，515 秒 |
| 耗时 | — | 分镜 120 秒（4 批）、渲染 139 秒 |

### 画面与音频脱节问题（已修复）

**现象**：用户反馈"嘉宾讲的内容与画面呈现的内容没匹配上，衔接不紧凑"，但画面本身质量可以。

**量化**：8.6 分钟音频（202 段字幕）实测 **93 段 / 238 秒 = 46% 的时长未被任何场景覆盖** → 这些时段画面停留在前一个场景，与正在讲的内容脱节。例：23~35s 音频在讲"9月18日凌晨两点宣布加息25个基点"，画面却仍是《加息与市场反应》。

**根因**：`MAX_SCENES = 24` 的抽稀上限过低（LLM 实际产出约 45~52 个场景），**一半场景被丢弃**；且丢弃发生在"空隙填补"之后 → 留下大量未覆盖区间。

**修复**（`src/scene.rs`）：
- `MAX_SCENES` 24 → **60**（长音频保持约 8~12 秒一个画面锚点，不再大量丢弃）
- 新增**空洞补齐** `fill_gaps()`：保证场景在条目维度**连续覆盖**
  - 小空洞（≤7 秒或 ≤2 段）并入前一个场景
  - 大空洞**按该段真实字幕合成一张金句卡**（内容取自该时段对话，保证画面与音频一致）
  - 已达场景上限时改为并入，避免无界增长
- 提示词收紧：要求**每个场景 `from_entry = 上一个场景 to_entry + 1`（连续拼接）**，单场景 6~14 秒，每批 8~14 个
...
| 场景数 | 24 | **52** |
| 平均场景时长 | 11.5s | **9.9s** |
| **未覆盖字幕段** | **93 段 / 238s / 46%** | **0 段 / 0s / 0%** |
| 类型分布 | 6 种 | cover / bullets×34 / quote×5 / chapter×5 / metric×4 / bar_chart / end×2 |

### 首帧黑屏（封面图未生效）问题（已修复）

**现象**：用户反馈"视频封面没有用生成的那张配图"。

**根因**：封面场景有「从 opacity 0 淡入 0.35 秒」的动画，因此**视频第一帧是纯背景（黑）**——
而视频的缩略图/封面正是首帧，且场景切换处也各有一帧纯背景。

**修复**：
- 首个场景**不做淡入**（`tl.set(opacity:1, 0)`），保证**第一帧就是配图**
- 场景片段向后延伸 `CLIP_OVERLAP_SECONDS = 0.4s`（末段不延伸），淡出与下一场景淡入**重叠** → 交叉淡化，消除切换瞬间的暗帧

**验证**：修复前首帧 46 色 / 平均 RGB(15,18,26)（黑）；修复后 **139 色 / 平均 RGB(50,58,61)**（配图），且 t=0、0.2s、1s 完全一致。
